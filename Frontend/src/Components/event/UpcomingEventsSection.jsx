...
// --- MOCK DATA GENERATOR (100 Events to test pagination) ---
const generateMockEvents = () => {
  const events = [];
  const types = ["workshop", "seminar", "hackathon", "bootcamp"];
  const speakers = ["Kaushik Sarkar", "Rahul Verma", "Priya Sharma", "Alex Chen"];
  
  for (let i = 1; i <= 50; i++) {
    const isUpcoming = i <= 20; // First 20 are upcoming
    const type = types[i % 4];
    
    events.push({
      _id: `evt-${i}`,
      title: isUpcoming ? `Upcoming Event ${i}: ${type.toUpperCase()}` : `Past Event ${i}: Legacy Code`,
      description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
      speakers: [speakers[i % 4]],
      type: type,
      venue: isUpcoming ? "Shyam Ji Audi" : "Lab Complex 2",
      date_from: isUpcoming 
        ? new Date(2026, 10, 10 + i).toISOString() 
        : new Date(2023, 5, i).toISOString(),
      date_to: isUpcoming 
        ? new Date(2026, 10, 10 + i, 12).toISOString() 
        : new Date(2023, 5, i, 12).toISOString(),
      ...
      photos: !isUpcoming ? ["https://placehold.co/600x400", "https://placehold.co/600x400"] : [],
      gdgUrl: "#",
      maxParticipants: 100,
      participantCount: isUpcoming ? Math.floor(Math.random() * 100) : 100,
    });
  }
  
  // Sort Logic: Upcoming (Asc) then Past (Desc)
...
const ALL_MOCK_DATA = generateMockEvents();
...
const getEventTypeColor = (type) => {
  switch (type) {
    ...
    case "bootcamp": return colors.red;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl bg-white rounded-3xl shadow-2xl overflow-hidden animate-scale-up max-h-[90vh] flex flex-col">
        <div className="relative h-56 flex-shrink-0 bg-gray-200">
          ...
          <button onClick={onClose} className="absolute top-4 right-4 w-10 h-10 bg-black/30 hover:bg-black/50 backdrop-blur-md rounded-full flex items-center justify-center text-white transition-all z-10">
            <FaTimes />
          </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {event.photos.map((photo, idx) => (
                      <div key={idx} className="aspect-square rounded-lg overflow-hidden shadow-sm bg-gray-100">
                        <img src={photo} alt="Gallery" className="w-full h-full object-cover hover:scale-110 transition-transform duration-500" />
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="space-y-4">
               <div className="p-5 bg-gray-50 rounded-xl border border-gray-100 space-y-4">
                  <div>
                    <div className="text-xs font-bold text-gray-400 uppercase">Speakers</div>
                    <div className="font-medium text-gray-800">{event.speakers.join(", ")}</div>
                  </div>
                  <div>
                    <div className="text-xs font-bold text-gray-400 uppercase">Venue</div>
                    <div className="font-medium text-gray-800">{event.venue}</div>
                  </div>
                  {isUpcoming && (
                     <a href={event.gdgUrl} target="_blank" rel="noreferrer" className="block w-full py-3 text-center text-white font-bold rounded-lg shadow-lg hover:opacity-90 transition-opacity" style={{ backgroundColor: themeColor }}>
                       Register on GDG
                     </a>
                  )}
               </div>
    ...
    >
      <div className="relative h-48 overflow-hidden flex-shrink-0 bg-gray-200">
        <img
          src={event.cover}
          alt={event.title}
          className={`w-full h-full object-cover transition-transform duration-500 group-hover:scale-110 ${!isUpcoming ? 'grayscale-[0.8] group-hover:grayscale-0' : ''}`}
        />
        ...
        <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm rounded-lg p-2 text-center shadow-lg">
          <div className="text-sm font-bold text-gray-900">{new Date(event.date_from).getDate()}</div>
          <div className="text-xs text-gray-600">{new Date(event.date_from).toLocaleString("default", { month: "short" })}</div>
        </div>
      </div>
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className="flex items-center space-x-2 mb-3">
          ...
          <span className="text-sm text-gray-600 truncate">By {event.speakers[0]}</span>
        </div>
          ...
          {isUpcoming ? (
            <>
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div className="h-2 rounded-full transition-all duration-500" style={{ width: `${percentFilled}%`, backgroundColor: themeColor }}></div>
              </div>
            </>
          ) : (
        ...
        <button
          ...
          style={{ backgroundColor: isUpcoming ? themeColor : '#e5e7eb', color: isUpcoming ? "white" : '#6b7280' }}
        >
          {isUpcoming ? (
            <><span>Register Now</span><FaArrowRight className="text-sm" /></>
          ) : (
            <><span>View Recap</span><FaExternalLinkAlt className="text-sm" /></>
          )}
        </button>
      </div>
      <div className="absolute inset-0 border-2 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" style={{ borderColor: isUpcoming ? themeColor : '#9ca3af' }}></div>
    </div>
  );
};
...
const Pagination = ({ currentPage, totalPages, onPageChange, isLoading }) => {
  // Logic to show truncated page numbers (e.g., 1 ... 4 5 6 ... 20)
...
const EventsSection = () => {
  ...
  const ITEMS_PER_PAGE = 9; // Display 9 items per page
  ...
  const fetchEvents = (pageNumber) => {
    ...
    setTimeout(() => {
        ...
        const paginatedData = ALL_MOCK_DATA.slice(startIndex, endIndex);
        
        setEvents(paginatedData);
        setTotalPages(Math.ceil(ALL_MOCK_DATA.length / ITEMS_PER_PAGE));
        setIsLoading(false);
    }, 600); // Faster load for better UX demo
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center space-x-2 bg-green-50 px-6 py-3 rounded-full border border-green-200 mb-6">
            ...
            <span className="text-lg font-semibold text-green-600">Community Calendar</span>
          </div>
          <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-4">
            Discover <span className="bg-gradient-to-r from-green-500 to-blue-500 bg-clip-text text-transparent">Amazing Events</span>
          </h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
             Browse our upcoming workshops or check out the gallery from past sessions.
          </p>
        </div>

        {/* Events Grid */}
        <div id="events-grid" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-8 min-h-[600px]">
          {isLoading 
            ? Array.from({ length: ITEMS_PER_PAGE }).map((_, i) => (
                <div key={i} className="bg-white rounded-2xl shadow-lg h-96 animate-pulse border border-gray-100 p-4">
